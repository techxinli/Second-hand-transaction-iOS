...
- (instancetype)initWithStartPoint:(CGPoint)startPoint startImage:(UIImage *)startImage submenuImages:(NSArray *)images
{
    if (self = [super init]) {
        ...
        self.start.layer.cornerRadius = self.start.frame.size.width/2;
        self.start.backgroundColor = [UIColor colorWithRed:1.0 green:0 blue:1.0 alpha:0.1];
...
- (void)commonSetup
{
    ...
    for (int i = 0; i < self.count; i++)
    {
        ...
        item.backgroundColor = [UIColor colorWithRed:1.0 green:0.6 blue:0.7 alpha:1];
...
{
    CGFloat firstAngle = M_PI + (M_PI_2 - kAngleOffset) + index * kAngleOffset -19.65;
